...
pub fn generate_level<T: Rng>(size: &usize, rng: &mut T) -> Vec<Cell> {
  ...
  let tractor = *tractor_candidates.choose(rng).unwrap();
  ...
  for _ in 0..n_holes {
    ...
    while let Some(candidate) = sorted_candidates.iter().choose(rng).cloned() {
